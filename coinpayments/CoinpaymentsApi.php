...
class CoinpaymentsApi
{
    ...
    const WEBHOOK_NOTIFICATION_URL = 'notifications';
     ...
     * @return string
     */
    public function getNotificationUrl($callback_url, $client_id, $event)
    {
        return $callback_url . '?clientId=' . $client_id . '&event=' . $event;
    }
    ...
    protected function appendInvoiceMetadata($request_data)
    {
        ...
        $request_data['metadata'] = array(
            "integration" => 'amemberPro ' . Am_Paysystem_Coinpayments::PLUGIN_REVISION,
            "hostname" => Am_Di::$instance->request->getHttpHost(),
        );
    ...
    function append_billing_data($request_data, $billing_data)
    {
        $request_data['buyer'] = array(
            "name" => array(
                "firstName" => $billing_data["name_f"],
                "lastName" => $billing_data['name_l']
            ),
        );
